#!/usr/bin/env tsx
/**
 * execution/ingest-supuestos-auxilio.ts
 *
 * Inserts parsed Auxilio Judicial supuestos oficiales into free_supuesto_bank + supuesto_bank.
 *
 * Data sources:
 *   - 2024: data/examenes_auxilio/2024_ej2/2024/parsed_b.json (2 supuestos)
 *   - 2025: data/examenes_auxilio/2025_ej2/2025/parsed_supuestos.json (2 supuestos)
 *
 * Rules:
 *   - 2024 Supuesto I → free_supuesto_bank (free users see this one)
 *   - ALL 4 supuestos → supuesto_bank (premium bank), dedup by fuente
 *
 * Usage: pnpm ingest:supuestos-auxilio [--dry-run]
 *
...
const OPOSICION_ID_AUXILIO = 'e0000000-0000-0000-0000-000000000001'
...
function transformCaso(supuesto: RawSupuesto) {
  ...
  return {
    ...
    escenario,
    bloques_cubiertos: ['procesal-civil', 'procesal-penal'],
  }
}
...
function buildFuente(anno: number, supuestoId: string): string {
  return `AUXILIO-${anno}-L-${supuestoId}`
}
...
const DATA_SOURCES: DataSource[] = [
  {
    filePath: path.join(__dirname, '..', '..', 'data', 'examenes_auxilio', '2024_ej2', '2024', 'parsed_b.json'),
    label: '2024 Auxilio (Modelo B)',
  },
  {
    filePath: path.join(__dirname, '..', '..', 'data', 'examenes_auxilio', '2025_ej2', '2025', 'parsed_supuestos.json'),
    label: '2025 Auxilio',
  },
]
...
async function main() {
  ...
  console.log(`Oposición ID (Auxilio): ${OPOSICION_ID_AUXILIO}\n`)
  ...
  // ── 1. Insert 2024 Supuesto I into free_supuesto_bank ─────────────────────
  const first2024 = allSupuestos.find(s => s.anno === 2024 && s.supuesto.id === 'SupuestoI')
  if (!first2024) {
    console.error('❌ Could not find 2024 SupuestoI for free bank')
    process.exit(1)
  }

  const freeRecord = {
    oposicion_id: OPOSICION_ID_AUXILIO,
    caso: transformCaso(first2024.supuesto),
    preguntas: transformPreguntas(first2024.supuesto),
    es_oficial: true,
    fuente: buildFuente(first2024.anno, first2024.supuesto.id),
  }

  const nonAnuladasFree = freeRecord.preguntas.length
  console.log(`📋 free_supuesto_bank: "${first2024.supuesto.titulo}" (${nonAnuladasFree} preguntas, ${first2024.supuesto.preguntas.length - nonAnuladasFree} anuladas filtered)`)
