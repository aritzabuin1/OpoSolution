 ...
 *   STRIPE_PRICE_FOUNDER      → Fundador C2               24.99€  one-time  → +30 análisis + badge
 *   STRIPE_PRICE_FOUNDER_C1   → Fundador C1               24.99€  one-time  → +30 análisis + badge
 *
...
export const STRIPE_PRICES = {
  ...
  fundador:     process.env.STRIPE_PRICE_FOUNDER      ?? '', // 24.99€ → C2 + badge
  fundador_c1:  process.env.STRIPE_PRICE_FOUNDER_C1   ?? '', // 24.99€ → C1 + badge
} as const
...
export const TIER_TO_OPOSICION: Record<StripePriceTier, string | 'doble'> = {
  ...
  fundador:     C2_OPOSICION_ID,
  fundador_c1:  C1_OPOSICION_ID,
}
